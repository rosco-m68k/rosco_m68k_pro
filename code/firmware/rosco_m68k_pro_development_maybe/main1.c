...
void print_longword(uint32_t addr) {
    uint32_t *ptr = (uint32_t*)addr;
    print_unsigned(*ptr, 16);    
    FW_PRINT_C("\r\n");
}

volatile uint16_t *tst = (uint16_t*)0x41e;
...
noreturn void default_program_loader() {
    ...
    FW_PRINT_C("!!! STAGE TWO DECOMPRESSED: First longword is 0x");
    print_longword(0x2000);

    FW_PRINT_C("!!! SDB_CPUINFO is 0x");
    print_longword(0x41c);

    *tst = 0xaaaa;
    
    FW_PRINT_C("!!! SDB_CPUINFO (CHANGED)  is 0x");
    print_longword(0x41c);
    
    *tst = 0x0001;
    
    FW_PRINT_C("!!! SDB_CPUINFO (REVERTED) is 0x");
    print_longword(0x41c);
